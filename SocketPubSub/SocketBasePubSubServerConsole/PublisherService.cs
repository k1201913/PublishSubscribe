...
namespace PubSubServerConsole
{
    class PublisherService
    {
        ...
        ~PublisherService()
        {
            //System.Console.WriteLine("Closing PublisherService");
            server.Close();
        }
        ...
        private void HostPublisherService()
        {
            ...
            IPAddress ipV4 = ReturnMachineIP();
        ...
        public static IPAddress ReturnMachineIP()
        {
            ...
            if (ipV4 == null)
            {
                System.Console.WriteLine("You have no IP of Version 4.Server can not run witout it");
                return null;
            }
            return ipV4;
        }
        ...
        private static void StartListening(Socket server)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    String topicName = messageParts[2];
                    if (!string.IsNullOrEmpty(command))
                    {
                        if (messageParts[0] == "Publish")
                        {
                            System.Console.WriteLine(command + " " + topicName);
                            if (!string.IsNullOrEmpty(topicName))
                            {
                                ...
                                HistoryFile.UpdateHistory(message);
